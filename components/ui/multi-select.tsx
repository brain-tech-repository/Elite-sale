...
export const MultiSelect = React.forwardRef<MultiSelectRef, MultiSelectProps>(
	(
		...
		const [selectedOptionMap, setSelectedOptionMap] = React.useState<
		...
		return (
			<>
				<div className="sr-only">
					{/* biome-ignore lint/a11y/useSemanticElements: role=alert is intentional for screen readers */}
				...
				>
					...
					<PopoverTrigger asChild>
						<Button
							...
							className={cn(
								" shadow-lg flex h-auto min-h-10 items-center justify-between rounded-md border bg-inherit p-1 hover:bg-inherit [&_svg]:pointer-events-auto",
									...
									>
										{selectedValues
											...
											.map((value) => {
												...
												const option =
													selectedOptionMap[value] || getOptionByValue(value);
												...
												return (
													...
													>
														...
														{/* biome-ignore lint/a11y/useSemanticElements: role=button is intentional here */}
														<div
															role="button"
															tabIndex={0}
															onClick={(event) => {
					...
					>
						<Command shouldFilter={false}>
							{searchable && (
								<CommandInput
									...
									onValueChange={(value) => {
										...
										onSearchChange?.(value); // 🔥 server search trigger
									}}
							...
							<CommandList
								key={`${searchValue}-${filteredOptions.length}`}
								className={cn(
