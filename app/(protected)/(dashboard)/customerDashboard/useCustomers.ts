...
export const useRegionPerformance = (filters?: SalesFilterPayload) =>
  useQuery({
    queryKey: ["customer-by-region", filters],
    ...
    queryFn: async () => {
      ...
      const { data } = await api.get(`/dashboard/customer/by-region?${query}`);
